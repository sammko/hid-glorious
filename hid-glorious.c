...
/*
 *  USB HID quirks support for Glorious PC Gaming Race
 *  Glorious Model O and O-.
 *
 ...
 * report descriptor, which leads to inputs being ignored.
 */
static __u8 *glorious_report_fixup(struct hid_device *hdev, __u8 *rdesc,
		unsigned int *rsize)
{
    if (*rsize == 213 && rdesc[84] == 129 && rdesc[112] == 129 && rdesc[140] == 129 &&
...
static int glorious_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
    int ret;
...
static const struct hid_device_id glorious_devices[] = {
	{ HID_USB_DEVICE(USB_VENDOR_ID_GLORIOUS, USB_DEVICE_ID_GLORIOUS_MODEL_O) },
	{ }
};
